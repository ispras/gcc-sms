                                     ...
                                     ddg_node_ptr dest_node, dep_t link)
{
  ...
  if (flag_modulo_sched_allow_regmoves 
      ...
      && !autoinc_var_is_used_p (dest_node->insn, src_node->insn))
    {
      ...
      for (def = DF_INSN_DEFS (dest_node->insn); def;
	   def = DF_REF_NEXT_LOC (def))
	{
	  df_ref first_def, use;
	  unsigned regno = DF_REF_REGNO (def);
	  struct df_rd_bb_info *bb_info = DF_RD_BB_INFO (g->bb);
	  bool src_uses = false;

	  for (use = DF_INSN_USES (src_node->insn); use;
	       use = DF_REF_NEXT_LOC (use))
	    if (regno == DF_REF_REGNO (use))
	      src_uses = true;
...
add_cross_iteration_register_deps (ddg_ptr g, df_ref last_def)
{
  ...
  if (flag_checking && DF_REF_ID (last_def) != DF_REF_ID (first_write_def))
    {
      struct df_rd_bb_info *bb_info = DF_RD_BB_INFO (g->bb);
      gcc_assert (!bitmap_bit_p (&bb_info->gen, DF_REF_ID (first_write_def)));
    }
